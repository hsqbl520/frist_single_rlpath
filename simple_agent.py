...
class MAAgent(parl.Agent):
    def __init__(self,
        ...
        self.min_memory_size = self.batch_size * 25  # batch_size * args.max_episode_len
        self.rpm = ReplayMemory(
            max_size=self.memory_size,
            obs_dim=self.obs_dim_n,
            act_dim=self.act_dim_n)
    ...
    def learn(self, agents):
        ...
        self.global_train_step += 1
            ...
            #return 0.0
        #print(self.global_train_step)
        if self.rpm.size() <= self.min_memory_size:
            return 0.0
        ##从经验池中读取数据，这三个分别是当前环境状态量、根据当前环境状态做出的动作、做出动作后的环境状态。
        batch_obs_n = []
        batch_act_n = []
        batch_obs_next_n = []
        ...
        for i in range(self.n):
            batch_obs, batch_act, _, batch_obs_next, _ \
                = agents[i].rpm.sample_batch_by_index(rpm_sample_index)# 每个Agent单独采样
